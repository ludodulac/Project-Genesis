...
export class PetitMondeLabScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.title = this.add.text(18, 14, 'PETIT MONDE — laboratoire', {
  ...
  private renderWorld(): void {
    ...
    for (const id of placeIds) {
      ...
      if (id === 'river') {
        this.graphics.fillStyle(0x86c5da, 0.72).fillRoundedRect(box.x + 103, box.y + 6, 38, box.h - 12, 18);
      }
      const vegetationMarks = Math.round(p.vegetation * 10);
      this.graphics.fillStyle(0x4d7c4a, 0.72);
      for (let i = 0; i < vegetationMarks; i += 1) {
        const px = box.x + 12 + ((i * 37) % 125);
        const py = box.y + 45 + ((i * 29) % 105);
    ...
    this.children.list.filter((child) => child.getData('worldLabel')).forEach((child) => child.destroy());
    for (const id of placeIds) {
      const p = this.state.places[id];
      const box = positions[id];
      ...
      label.setData('worldLabel', true).setInteractive({ useHandCursor: true });
    ...
    for (const placeId of placeIds) {
      ...
      occupants[placeId].forEach((id, index) => {
        ...
        name.setData('worldLabel', true).setInteractive({ useHandCursor: true });
