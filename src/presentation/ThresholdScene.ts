...
import { EXP028_START, cloneBoard, tap, type Board } from '../experiments/exp028-threshold';
...
export class ThresholdScene extends Phaser.Scene {
  ...
  private board: Board = cloneBoard(EXP028_START);
  private busy = false;
  ...
  private async play(row: number, col: number): Promise<void> {
    ...
    if (this.board.every(line => line.every(cell => cell === 0))) {
      ...
      this.board = cloneBoard(EXP028_START);
      this.redraw();
    }
